...
export default function Navbar() {
  ...
  const { toggleCart, mobileMenuOpen, toggleMobileMenu, setMobileMenuOpen } = useUIStore();
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const { data } = await api.get(`/products?search=${encodeURIComponent(debouncedSearch)}&limit=5`);
        if (data.success) {
  ...
  return (
    <>
      ...
      <div className="bg-zinc-900 text-white text-[11px] font-semibold py-1.5 z-50 relative">
        <div className="site-container flex items-center justify-center sm:justify-between">
          <span className="flex items-center gap-1.5 tracking-wide text-zinc-300">
            <Sparkles className="w-3 h-3 text-amber-400" /> Free Delivery on orders above ৳2,000
          </span>
          <Link href="/track" className="hidden sm:flex items-center gap-1 text-zinc-400 hover:text-white transition-colors tracking-wide uppercase text-[10px]">
      ...
      <header className="sticky top-0 z-40 w-full bg-white border-b border-gray-100 shadow-sm">
        ...
        <div className="site-container flex items-center justify-between h-[60px] gap-6">

          {/* Logo & Mobile Toggle */}
          <div className="flex items-center gap-3">
            <button onClick={toggleMobileMenu} className="md:hidden flex items-center justify-center text-gray-800 -ml-2 p-2 focus:outline-none">
            ...
            <form onSubmit={handleSearchSubmit} className="relative group">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value.trim().length > 1) setShowSearchDropdown(true);
                }}
                onFocus={() => {
              ...
              <AnimatePresence>
                {showSearchDropdown && search.trim().length > 1 && (
                  <motion.div
                    ...
                    className="absolute left-0 right-0 top-[calc(100%+4px)] bg-white border border-gray-100 shadow-xl overflow-hidden z-50 flex flex-col max-h-[350px]"
                  >
                      ...
                      {isSearching && searchResults.length === 0 ? (
                        ...
                        <>
                          {searchResults.map((product) => (
                            ...
                            >
                              <img src={getProductImage(product)} alt={product.name} className="w-10 h-10 object-cover bg-white border border-gray-100 p-0.5" />
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <button onClick={() => router.push("/products")} className="md:hidden flex items-center justify-center p-2 text-gray-800 hover:bg-gray-50 transition-colors">
              <Search className="w-4 h-4" />
            </button>

            <Link href="/wishlist" className="hidden sm:flex items-center justify-center p-2 text-gray-800 hover:bg-gray-50 transition-colors">
              <Heart className="w-4 h-4" />
            </Link>

            {/* Cart Icon */}
            <button onClick={toggleCart} className="relative flex items-center justify-center p-2 text-gray-800 hover:bg-gray-50 transition-colors">
              <ShoppingCart className="w-4 h-4" />
              <AnimatePresence>
                {itemCount > 0 && (
                  <motion.span
                    initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                    className="absolute top-1 right-0 w-[15px] h-[15px] bg-[#ef4a23] text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-sm"
                  >
            ...
            {isAuthenticated ? (
                  ...
                  {showUserMenu && (
                    ...
                    >
                      <div className="px-3 py-2.5 mb-1 bg-gray-50 border-b border-gray-100">
                        <p className="font-bold text-[13px] truncate text-gray-900">{user?.full_name || "User Account"}</p>
                        <p className="text-[10px] text-gray-500 truncate mt-0.5">{user?.email}</p>
                      </div>
                      <Link href="/orders" className="flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium text-gray-700 hover:text-black hover:bg-gray-50 transition-colors">
                        <Package className="w-3.5 h-3.5" /> My Orders
                      </Link>
                      <Link href="/wishlist" className="flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium text-gray-700 hover:text-black hover:bg-gray-50 transition-colors">
                        <Heart className="w-3.5 h-3.5" /> Wishlist
                      </Link>
                      {user?.role === "admin" && (
                        <Link href="/admin" className="flex items-center gap-2.5 px-3 py-2 text-[12px] font-bold text-[#ef4a23] hover:bg-red-50 transition-colors">
                          <Sparkles className="w-3.5 h-3.5" /> Admin Panel
                        </Link>
                      )}
                      <div className="h-px bg-gray-100 my-1 mx-2" />
                      <button onClick={() => { logout(); setShowUserMenu(false); }} className="flex items-center gap-2.5 px-3 py-2 text-[12px] font-bold text-red-600 hover:bg-red-50 transition-colors w-full text-left">
              ...
              <Link href="/login" className="bg-black hover:bg-[#ef4a23] text-white py-1.5 px-4 text-[11px] font-bold tracking-wide uppercase transition-colors ml-1">
                Sign In
              </Link>
            )}
          </div>
        ...
        {!isAdmin && (
          <div className="hidden md:block border-t border-gray-100 bg-gray-50/50">
            <div className="site-container flex items-center justify-between h-[42px]">
                  ...
                  <AnimatePresence>
                    {showCategories && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 5 }}
                        transition={{ duration: 0.15 }}
                        className="absolute left-0 top-[100%] w-64 bg-white border-x border-b border-gray-100 shadow-[0_10px_20px_rgb(0,0,0,0.05)] z-50 py-1 max-h-[60vh] overflow-y-auto"
                      >
                        {categories.map((cat) => (
                          <Link
                            key={cat.id}
                          ...
                          >
                            {cat.image_url ? (
                              <img src={cat.image_url} alt="" className="w-6 h-6 object-contain filter grayscale group-hover:grayscale-0 transition-all opacity-80 group-hover:opacity-100" />
                            ) : (
                              <span className="w-6 h-6 bg-gray-100 flex items-center justify-center text-[10px]">📦</span>
                            )}
                            <span className="font-bold text-[12px] text-gray-700 group-hover:text-black">{cat.name}</span>
                          </Link>
                        ))}
                      </motion.div>
                    )}
              ...
              <Link href="/deals" className="text-[12px] font-bold text-[#ef4a23] hover:text-red-700 uppercase tracking-widest flex items-center h-full">
                 Today's Deals
              </Link>
            </div>
          </div>
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-gray-50/50">
                ...
                <button onClick={() => setMobileMenuOpen(false)} className="p-1.5 text-gray-500 hover:bg-gray-200 transition-colors rounded-sm">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto flex-1 p-5 pb-20">
                ...
                <div className="flex flex-col gap-1 mb-6 border-b border-gray-100 pb-6">
                  <Link href="/products" onClick={() => setMobileMenuOpen(false)} className="text-[14px] font-bold text-gray-900 py-2.5 hover:text-[#ef4a23]">All Products</Link>
                  <Link href="/products?sort_by=created_at&sort_order=desc" onClick={() => setMobileMenuOpen(false)} className="text-[14px] font-bold text-gray-900 py-2.5 hover:text-[#ef4a23]">New Arrivals</Link>
                  <Link href="/deals" onClick={() => setMobileMenuOpen(false)} className="text-[14px] font-bold text-[#ef4a23] py-2.5">Today's Deals</Link>
                </div>

                {/* User & Utils */}
                <div className="flex flex-col gap-1 mb-8">
                  <Link href="/wishlist" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-2 text-[13px] font-semibold text-gray-600 hover:text-black">
                    <Heart className="w-4 h-4" /> Wishlist
                  </Link>
                  <Link href="/orders" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-2 text-[13px] font-semibold text-gray-600 hover:text-black">
                    <Package className="w-4 h-4" /> My Orders
                  </Link>
                  <Link href="/track" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 py-2 text-[13px] font-semibold text-gray-600 hover:text-black">
                ...
                {categories.length > 0 && (
                  <div>
                    <h4 className="text-[11px] font-bold text-gray-400 tracking-widest uppercase mb-3 px-1">Categories</h4>
              ...
              {!isAuthenticated && (
                <div className="border-t border-gray-100 p-4 bg-gray-50/50 mt-auto">
                   <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="block w-full bg-black text-white text-center py-3 text-[13px] font-bold uppercase tracking-wider hover:bg-[#ef4a23] transition-colors">
                      Sign In / Register
                   </Link>
                </div>
              )}
